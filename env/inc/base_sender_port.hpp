...
private:
  ...
  template <typename Function> env_status_t<this_t> set_writer_(Function function) {
  ...
  env_status_t<this_t> reset_writer_() {
    env_status_t<this_t> temp_status(this);
  ...
  env_data_t<data_t, this_t> dequeue_data_() {
    env_data_t<data_t, this_t> temp_status(this);

    if (buffer_.empty()) {

      temp_status.status.qualifiers.insert(std::make_pair(this->get_id(), env_errno_t::ENV_BUFFER_EMPTY));
      ...
      temp_status.status.qualifiers.insert(std::make_pair(this->get_id(), env_errno_t::ENV_CLEAR));
      buffer_.erase(buffer_.begin());
      return temp_status;
    }
  }

  env_status_t<this_t> forward_() {
    env_status_t<this_t> temp_status(this);
    ...
    if (transaction_count_) {
      ...
      while (transaction_count_) {
        ...
        for (port_info_t &connected : this->connected()) {

          env_data_t<data_t, this_t> temp_data = dequeue_data_();

          if (temp_data.status.qualifiers.at(this->get_id()) == env_errno_t::ENV_CLEAR) {
            ...
            // != env_errno_t::ENV_CLEAR ){ /* TODO: change connection
            ...
            //   temp_status.qualifiers.insert( std::make_pair( this->get_id(),
            //   env_errno_t::ENV_CLEAR )); connected.second->on_receive()(
...
public:
  ...
  template <typename Function> env_status_t<this_t> set_writer(Function function) { return set_writer_(function); }

  env_status_t<this_t> reset_writer() { return reset_writer_(); }
  ...
  const hook_t<void(const data_t &)> &on_send() const { return on_send_(); }
  const functor_t<data_t &(data_t &), this_t> &writer() const { return writer_(); }
  env_status_t<this_t> forward() { return forward_(); }

  template <typename NameType>
  explicit base_sender_port_t(const NameType &name)

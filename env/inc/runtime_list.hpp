...
template <uint32_t N>
struct runtime_list_static_t : std::array<std::tuple<sha256::sha256_hash_type, sha256::sha256_hash_type,
                                                     void (*)(int, char **, const struct env_base_t *)>,
                                          N> {
public:
  static constexpr uint32_t size = N;
  using runtime_t = void (*)(int argc, char **argv, const struct env_base_t *env_);
  using base_t = std::array<
      std::tuple<sha256::sha256_hash_type, sha256::sha256_hash_type, void (*)(int, char **, const struct env_base_t *)>,
      N>;

  explicit runtime_list_static_t(const std::tuple<std::string, std::string, runtime_t> (&runtimes)[N])
  ...
  explicit runtime_list_static_t(std::tuple<std::string, std::string, runtime_t> (&runtimes)[N])
      : std::array<std::tuple<sha256::sha256_hash_type, sha256::sha256_hash_type, runtime_t>, N>() {
    for (uint32_t i = 0u; i < N; i++)
      (*this).at(i) =
          std::make_tuple(sha256::compute(reinterpret_cast<const uint8_t *>(std::get<0u>(runtimes[i]).data()),
  ...
  virtual ~runtime_list_static_t() = default;
  void operator()(int argc, char *argv[], const struct env_base_t *p_env) const {
  ...
  template <typename NameType> runtime_t operator[](const NameType &name) {

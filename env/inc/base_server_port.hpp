...
private:
  ...
  static constexpr bool is_async_ = (runtime_type == runtime_t::RUNTIME_ASYNC);
  ...
  template <typename Function> env_status_t<this_t> register_call_(const std::string &name, const Function &function) {
    static_assert(
  ...
  env_status_t<this_t> register_call_(const std::string &name, Class *p_class, const Function &function) {
    static_assert(
        std::is_same_v<typename std::function_traits::function_traits<Function>::function_t, ReturnType(Args...)> ||
            std::is_convertible_v<std::function<typename std::function_traits::function_traits<Function>::function_t>,
                                  std::function<ReturnType(Args...)>>,
  ...
  env_status_t<this_t> remove_call_(const std::string &name) {
    const sha256::sha256_hash_type hash =
        sha256::compute(reinterpret_cast<const uint8_t *>(name.data()), name.length());
    return remove_(hash);
  }

  env_status_t<this_t> register_(const sha256::sha256_hash_type &id, const std::function<ReturnType(Args...)> &function) {
    ...
    if (!call_exists_(id)) {
      ...
      temp_status.qualifiers.insert(std::make_pair(this->get_id(), env_errno_t::ENV_CALL_EXISTS));
  ...
  env_status_t<this_t> remove_(const sha256::sha256_hash_type &id) {
    env_status_t<this_t> temp_status(this);
    ...
    if (call_exists_(id)) {
      ...
      temp_status.qualifiers.insert(std::make_pair(this->get_id(), env_errno_t::ENV_CALL_MISSING));
      this->error_handler()(this->get_id(), temp_status.qualifiers.at(this->get_id()),
                            error_case_t::ERROR_CASE_RUNTIME);
      return temp_status;
    }
  ...
  env_status_t<this_t> create_response_(const sha256::sha256_hash_type &call_id, const std::tuple<Args...> &args) {
    env_status_t<this_t> temp_status(this);
  ...
  env_status_t<this_t> push_request_(const request_t_ &request) {
    env_status_t<this_t> temp_status(this);
...
public:
  ...
  template <typename Function> env_status_t<this_t> register_call(const std::string &name, const Function &function) {
    return register_call_(name, function);
  }

  template <typename Class, typename Function>
  env_status_t<this_t> register_call(const std::string &name, Class *p_class, const Function &function) {
    return register_call_(name, p_class, function);
  }

  env_status_t<this_t> remove_call(const std::string &name) { return remove_call_(name); }
  env_status_t<this_t> create_response(const sha256::sha256_hash_type &call_id, const std::tuple<Args...> &args) {
    return create_response_(call_id, args);
  }
  env_data_t<response_t_, this_t> get_response() const { return get_response_(); }
  env_status_t<this_t> push_request(const request_t_ &request) { return push_request_(request); }
  env_data_t<request_t_, this_t> get_request() const { return get_request_(); }

  template <typename NameType>
  explicit base_server_port_t(const NameType &name)
  ...
  explicit base_server_port_t(const NameType &name,
    ...
    for (unsigned int i = 0; i < sizeof...(functions); i++) {
      ...
      env_status_t<this_t> register_status = std::visit_at(
          i, [i, this](const auto &element) -> env_status_t<this_t> { return register_(element); }, temp_tp);

      if (register_status.qualifiers.at(this->get_id()) != env_errno_t::ENV_CLEAR) {

...
sha256::sha256_hash_type sha256::compute(const void *const data, const size_t size) {
  ...
  intmem::storeu_be(&LastBlock[56u], size << 3u);
  ::transform(State, LastBlock);
...
sha256::sha256_hash_type sha256::sha256_from_string(const std::string &data) {
  sha256_hash_type ret;
  std::memcpy(ret.data(), reinterpret_cast<const uint8_t *>(data.data()), ret.size());
  return std::move(ret);
}

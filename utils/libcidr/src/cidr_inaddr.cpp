...
struct in_addr *cidr_to_inaddr(const CIDR *addr, struct in_addr *uptr) {
  ...
  toret = uptr;
  if (toret == NULL)
    toret = (struct in_addr *)malloc(sizeof(struct in_addr));
  if (toret == NULL) {
    errno = ENOMEM;
    return (NULL);
  }

  std::memset(toret, 0, sizeof(struct in_addr));

  /* Add 'em up and stuff 'em in */
  toret->s_addr = ((addr->addr)[12] << 24) + ((addr->addr)[13] << 16) + ((addr->addr)[14] << 8) + ((addr->addr)[15]);
...
CIDR *cidr_from_inaddr(const struct in_addr *uaddr) {
  ...
  toret->mask[15] = toret->mask[14] = toret->mask[13] = toret->mask[12] = 0xff;
...
struct in6_addr *cidr_to_in6addr(const CIDR *addr, struct in6_addr *uptr) {
  ...
  toret = uptr;
  if (toret == NULL)
    toret = (struct in6_addr *)malloc(sizeof(struct in6_addr));
  if (toret == NULL) {
    errno = ENOMEM;
    return (NULL);
  }
  
  std::memset(toret, 0, sizeof(struct in6_addr));

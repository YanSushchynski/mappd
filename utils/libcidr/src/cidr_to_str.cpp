...
char *cidr_to_str(const CIDR *block, int flags) {
  ...
  if ((block->proto == CIDR_IPV4 && !(flags & CIDR_FORCEV6)) || (flags & CIDR_FORCEV4)) {
    ...
    if (flags & CIDR_REVERSE) {
      ...
      std::sprintf(toret, "%d.%d.%d.%d.in-addr.arpa", block->addr[15], block->addr[14], block->addr[13],
                   block->addr[12]);
      return (toret);
    }
      ...
      if (flags & CIDR_USEV6) {
        if (flags & CIDR_NOCOMPACT) {
          if (flags & CIDR_VERBOSE)
            std::strcat(toret, "0000:0000:0000:0000:0000:");
          else
            std::strcat(toret, "0:0:0:0:0:");
        } else
          std::strcat(toret, "::");
        ...
        if (flags & CIDR_USEV4COMPAT) {
          if (flags & CIDR_NOCOMPACT) {
            if (flags & CIDR_VERBOSE)
              std::strcat(toret, "0000:");
            else
              std::strcat(toret, "0:");
          }
        } else
          std::strcat(toret, "ffff:");
      } /* USEV6 */

      /* Now, slap on the v4 address */
      for (i = 12; i <= 15; i++) {
        std::sprintf(tmpbuf, "%u", (block->addr)[i]);
        std::strcat(toret, tmpbuf);
        if (i < 15)
          std::strcat(toret, ".");
      }
    } /* ! ONLYPFLEN */
    ...
    if (!(flags & CIDR_ONLYADDR)) {
      ...
      if (flags & CIDR_NETMASK) {
        ...
        std::sprintf(tmpbuf, "%u", (flags & CIDR_USEV6) ? pflen + 96 : pflen);
        std::strcat(toret, tmpbuf);
      }
    } /* ! ONLYADDR */

    /* That's it for a v4 address, in any of our forms */
  } else if ((block->proto == CIDR_IPV6 && !(flags & CIDR_FORCEV4)) || (flags & CIDR_FORCEV6)) {
    ...
    if (flags & CIDR_REVERSE) {
      ...
      std::sprintf(toret,
                   ...
                   "%x.%x.%x.%x.%x.ip6.arpa",
                   block->addr[15] & 0x0f, block->addr[15] >> 4, block->addr[14] & 0x0f, block->addr[14] >> 4,
                   block->addr[13] & 0x0f, block->addr[13] >> 4, block->addr[12] & 0x0f, block->addr[12] >> 4,
                   block->addr[11] & 0x0f, block->addr[11] >> 4, block->addr[10] & 0x0f, block->addr[10] >> 4,
                   block->addr[9] & 0x0f, block->addr[9] >> 4, block->addr[8] & 0x0f, block->addr[8] >> 4,
                   block->addr[7] & 0x0f, block->addr[7] >> 4, block->addr[6] & 0x0f, block->addr[6] >> 4,
                   block->addr[5] & 0x0f, block->addr[5] >> 4, block->addr[4] & 0x0f, block->addr[4] >> 4,
                   block->addr[3] & 0x0f, block->addr[3] >> 4, block->addr[2] & 0x0f, block->addr[2] >> 4,
                   block->addr[1] & 0x0f, block->addr[1] >> 4, block->addr[0] & 0x0f, block->addr[0] >> 4);
      return (toret);
    }
    ...
    if (!(flags & CIDR_ONLYPFLEN)) {
      ...
      for (i = 0; i <= 15; i += 2) {
         ...
         */
        if (flags & CIDR_VERBOSE)
          std::sprintf(tmpbuf, "%.4x", v6sect);
        else
          std::sprintf(tmpbuf, "%x", v6sect);
        std::strcat(toret, tmpbuf);

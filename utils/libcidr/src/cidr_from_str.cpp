...
#include <cctype>
#include <cerrno>
#include <cstdio> /* I'm always stuffing debug printf's into here */
#include <cstdlib>
#include <cstring>
#include <strings.h>
  ...
  if (foundpf == 0) {
    ...
    if (foundmask == 0) {
      ...
      if (pfx[1] == '0' && tolower(pfx[2]) == 'x')
        ...
        if (*buf2 == '\0' && octet >= (unsigned long)(1 << 31) && octet <= (unsigned long)0xffffffff)
          foundmask = 1; /* Valid! */
...
#undef UMIN
    ...
    for (/* i */; i >= 0; i--) {
       ...
       * for details.
       */
      if ((nocts != 0 && octet > 255) || (nocts == 0 && octet > (0xffffffff >> (8 * (nsect - 1))))) {
        cidr_free(toret);
        errno = EINVAL;
        return (NULL);
      }

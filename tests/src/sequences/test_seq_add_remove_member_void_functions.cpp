...
TEST(Sequence, add_remove_void_member_functions) {
  {
    class Test {
    public:
      int result;
      void slot(int a, int b) { this->result = a + b; }

      void increment(void) { this->result++; }
    };
...

  {
    ...
    for (unsigned int i = 0u; i < 10u; i++) {
      ...
      auto add_result = sequence_.add("Summa" + std::to_string(i), &test_, &Test::increment);
    ...
    EXPECT_EQ(test_.result, 5u + 5u + 10u);
    EXPECT_EQ(sequence_.clear().qualifiers.at(sequence_.get_id()), seq_errno_t::SEQ_CLEAR);
  }
}

...
TEST(Hook, add_remove_member_functions) {
  {
    class Test {
    public:
      void slot(int a, int b, std::string &captured_string) {
...

    {
      ...
      EXPECT_EQ(remove_result.data.first, sha256::compute(reinterpret_cast<const uint8_t *>("Summa"), std::strlen("Summa")));
...

  {
...

    {
      ...
      EXPECT_EQ(add_result.status.qualifiers.at(hook_.get_id()), hook_errno_t::HOOK_CLEAR);
      EXPECT_EQ(add_result.data.first, sha256::compute(reinterpret_cast<const uint8_t *>("Summa"), std::strlen("Summa")));

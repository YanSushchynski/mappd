...
TEST(Hook, replace_static_member_functions) {
  {
    class Test {
    public:
      static void slot1(int a, int b, std::string &captured_string) {
        captured_string += "FirstSumma = " + std::to_string(a + b);
      }

      static void slot2(int a, int b, std::string &captured_string) {
    ...
    {
      auto add_result = hook_.add("FirstSumma", Test::slot1);
    ...
    {
      auto replace_result = hook_.replace("FirstSumma", "SecondSumma", Test::slot2);
